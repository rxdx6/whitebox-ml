...
inline float avg_loss(const std::vector<float> &X, const std::vector<float> &y,
  ...
  float l2_penalty = 0.5 * lambda * w * w;
...
inline std::vector<float> train(const std::vector<float> &X,
  ...
  for (int e = 0; e < epochs; e++) {
    std::vector<float> w_and_b = update_w_and_b(X, y, w, b, alpha, lambda);
    w = w_and_b[0];
    b = w_and_b[1];

    if (e % 1000 == 0) {
      float current_loss = avg_loss(X, y, w, b, lambda);
      printf("Epoch %d - Current Log Loss: %f\n", e, current_loss);
    }
  }
  printf("Training complete. Final Log Loss: %f\n", avg_loss(X, y, w, b, lambda));
  return {w, b};
}
