...
update_w_and_b(const std::vector<std::vector<float>> &features_matrix,
               const std::vector<float> &target_labels, const std::vector<float> &weights,
  ...
  for (size_t sample_index = 0; sample_index < num_samples; sample_index++) {
    ...
    for (size_t feature_index = 0; feature_index < num_features; feature_index++) {
      predicted_value += weights[feature_index] * features_matrix[sample_index][feature_index];
    }

    float prediction_error = target_labels[sample_index] - predicted_value;
    for (size_t feature_index = 0; feature_index < num_features; feature_index++) {
      weight_gradients[feature_index] += -2 * features_matrix[sample_index][feature_index] * prediction_error;
    }
  ...
  for (size_t feature_index = 0; feature_index < num_features; feature_index++) {
    updated_weights[feature_index] -= learning_rate * (((1.0f / num_samples) * weight_gradients[feature_index]) + (l2_regularisation_strength * weights[feature_index]));
  }
  float updated_bias = bias - (1.0f / num_samples) * bias_gradient * learning_rate;
...
inline float avg_loss(const std::vector<std::vector<float>> &features_matrix,
                      const std::vector<float> &target_labels, const std::vector<float> &weights,
                      float bias, float l2_regularisation_strength) {
  float num_samples = features_matrix.size();
...
inline LinearRegressionResult train(const std::vector<std::vector<float>> &features_matrix,
  ...
  for (int epoch = 0; epoch < num_epochs; epoch++) {
    LinearRegressionResult regression_result =
        update_w_and_b(features_matrix, target_labels, current_weights, current_bias, learning_rate, l2_regularisation_strength);
...
inline std::vector<float> predict(const std::vector<std::vector<float>> &features_matrix,
                                  const std::vector<float> &weights, float bias) {
  ...
  for (size_t sample_index = 0; sample_index < features_matrix.size(); sample_index++) {
    float predicted_value = bias;
    for (size_t feature_index = 0; feature_index < weights.size(); feature_index++) {
      predicted_value += weights[feature_index] * features_matrix[sample_index][feature_index];
    }
